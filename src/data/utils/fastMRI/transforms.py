...
def resize_tensor(tensor, new_size=(256, 256)):
    ...
    """
    # Ensure the tensor has a batch and channel dimension
...
class UnetDataTransform:
    ...
    def __call__(
        ...
        image = resize_tensor(image, input_size)
        target_torch = resize_tensor(target_torch, input_size)
        return UnetSample(
            image=image.unsqueeze(0),
            target=target_torch.unsqueeze(0),
            mean=mean,
            std=std,
            fname=fname,
            slice_num=slice_num,
...
class MiniCoilTransform:
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        kspace = fft2c_new(
            complex_center_crop(ifft2c_new(to_tensor(kspace)), square_crop)
        ).numpy()
        ...
        target = rss_complex(ifft2c_new(to_tensor(kspace)))
